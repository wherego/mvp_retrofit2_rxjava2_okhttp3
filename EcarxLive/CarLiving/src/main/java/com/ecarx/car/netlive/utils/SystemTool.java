...
public final class SystemTool {
     ...
     * @return
     */
    public static void getDefaultHeight(Activity context) {
        int height;
        int top;
        Rect outRect1 = new Rect();
        context.getWindow().getDecorView().getWindowVisibleDisplayFrame(outRect1);
        height = outRect1.height() > 0 ? outRect1.height() : -1;
        top = outRect1.height() > 0 ? outRect1.top : -1;
        if (height > 0)
            SharedPreferencesUtils.getInstance(context).setInt(SharedPreferencesUtils.recover_height, height);
        if (top > 0)
            SharedPreferencesUtils.getInstance(context).setInt(SharedPreferencesUtils.status_height, top);
    }
